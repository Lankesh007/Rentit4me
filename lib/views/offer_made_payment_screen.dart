...
class OfferMadePaymentScreen extends StatefulWidget {
  ...
  const OfferMadePaymentScreen({this.postadid, this.offerid, Key key})
      : super(key: key);

  @override
  State<OfferMadePaymentScreen> createState() => _OfferMadePaymentScreenState();
}

class _OfferMadePaymentScreenState extends State<OfferMadePaymentScreen> {
  bool _loading = false;
  ...
  Future applyCouponDetails() async {
    ...
    var body = {
      ...
      "type": "rent".toString(),
      "sub_total": subTotalAmount.toString(),
      "convenience_fee": convinenceCharge.toString()
    };
    ...
    if (result['ErrorCode'] == 0) {
      setState(() {
        ...
        appliedGrandTotal = applidTotalAmount + double.parse(convinenceCharge);
  ...
  Future<void> _getofferdetailproduct() async {
    ...
    print(jsonEncode({
      "offer_request_id": widget.postadid,
    }));
    print(BASE_URL + offerdetail);
    final body = {"offer_request_id": widget.postadid};
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        postId = data['Product Details']['id'];
        productName = data['Product Details']['title'].toString();

        securitydeposit = data['Offer Details']['total_security'].toString();
        quantity = data['Offer Details']['quantity'].toString();
        currencyType = data['Product Details']['currency'].toString();
        price =
            data['Offer Details']['final_product_selling_amount'].toString();
        convinenceCharge = data['Offer Details']['convenience_fee'].toString();
        totalRent = data['Offer Details']['total_rent'].toString();
        totalsecurity = data['Offer Details']['total_security'].toString();

        subTotalAmount = double.parse(totalRent) + double.parse(totalsecurity);
        log("subtotalAmount==>$subTotalAmount");

        totalAmount = data['Offer Details']['final_amount'].toString();
        duration = data['Offer Details']['period'].toString();
